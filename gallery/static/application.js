...
return jQuery;}));$(function(){function keydownNavigate(direction){selector=$("[data-navigate="+direction+"]:last");if(selector.attr("href")){window.location=selector.attr("href");}}
$(document).keydown(function(event){if(event.which==37)(keydownNavigate("left"));if(event.which==39)(keydownNavigate("right"));if(event.which==40)(keydownNavigate("down"));if(event.which==38&&$(document).scrollTop()==0)(keydownNavigate("up"));});function enableModalWindow(){$("#modal-window").css('display','block');$("#modal .cancel").click(function(event){disableModalWindow();});$("#modal select").chosen();}
function disableModalWindow(){$("#modal").html("");$("#modal-window").css('display','none');}
function errorProcessing(data){console.log(data);alert("Error processing request. Please try again later.");}



function initializeModalForm(){$("#modal form").submit(function(event){event.preventDefault();$.ajax({method:"POST",url:$(this).attr("action"),data:$(this).serialize(),success:function(data){if(data.url){disableModalWindow();window.location=data.url;}
else if(data.html){$("#modal").html(data.html);enableModalWindow();initializeModalForm();}
else{errorProcessing(data);}},error:function(data){errorProcessing(data);}});});}
$("[data-modal='rotate']").click(function(event){event.preventDefault();$.ajax({method:"POST",url:$(this).attr("href"),success:function(data){img=$(".photo img");img.attr("src",img.attr("src").split("?")[0]+"?"+
new Date().getTime());},error:function(data){errorProcessing(data);}});});$("[data-modal='form']").click(function(event){event.preventDefault();$.getJSON($(this).attr("href"),function(data){if(data.html){$("#modal").html(data.html)
enableModalWindow();initializeModalForm();$("#modal input[type='text']:enabled:first").focus();}
else{errorProcessing(data);}});});});(function(){var $,AbstractChosen,Chosen,SelectParser,_ref,__hasProp={}.hasOwnProperty,__extends=function(child,parent){for(var key in parent){if(__hasProp.call(parent,key))child[key]=parent[key];}function ctor(){this.constructor=child;}ctor.prototype=parent.prototype;child.prototype=new ctor();child.__super__=parent.prototype;return child;};SelectParser=(function(){function SelectParser(){this.options_index=0;this.parsed=[];}
